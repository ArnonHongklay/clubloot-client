...
.controller 'ContestTemplateShowCtrl', ($scope, $http, Auth, $state, $stateParams) ->
  ...
  $.ajax(
    method: 'GET'
    url: "http://api.clubloot.com/contests/program/#{$stateParams.program_id}/template/#{$stateParams.template_id}.json"
    ).done (data) ->
